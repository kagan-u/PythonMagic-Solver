...
import uvicorn
from recaptcha_v2_solver import solve_recaptcha_v2 as _solve_recaptcha_v2_impl
from hcaptcha_solver import solve_hcaptcha as _solve_hcaptcha_impl
...
class ClearanceAPIServer:
    ...
    <head>
        ...
        <title>PythonMagic-Solver</title>
    ...
    def __init__(self, headless: bool, thread: int, page_count: int,
        ...
        self.app.get("/recaptchaV2")(self.process_recaptcha_v2)
        self.app.post("/recaptchaV2")(self.process_recaptcha_v2)
        self.app.get("/hcaptcha")(self.process_hcaptcha)
        self.app.post("/hcaptcha")(self.process_hcaptcha)
    ...
    async def _solve_recaptcha_v2_task(self, task_id, url, sitekey, version,
                                       enterprise, action, classifier):
        start_time = time.time()
        page, context = await self.page_pool.get()
        normalized = "invisible" if version.lower() in ("invisible", "v3", "execute") else "checkbox"
        try:
            res = await _solve_recaptcha_v2_impl(
                page, url, sitekey, enterprise=enterprise,
                version=normalized, action=action, classifier=classifier)
            elapsed = res.get("elapsed", round(time.time() - start_time, 3))
            token = res.get("token")
            if token:
                self.results[task_id] = {"status": "success", "elapsed_time": elapsed, "value": token}
                logger.success(f"[reCAPTCHA v2] Sukses ({res.get('method', '')}) — {task_id} ({elapsed}s)")
            else:
                self.results[task_id] = {
                    "status": "error", "elapsed_time": elapsed,
                    "value": "captcha_fail", "message": res.get("error", "gagal")}
                logger.warning(f"[reCAPTCHA v2] Gagal — {task_id}: {res.get('error')}")
        except Exception as e:
            elapsed = round(time.time() - start_time, 3)
            self.results[task_id] = {"status": "error", "elapsed_time": elapsed, "value": str(e)}
            logger.error(f"[reCAPTCHA v2] Exception — {task_id}: {e}")
        finally:
            try:
                await page.unroute_all()
            except Exception:
            ...
            if context in self.contexts:
                ...
                logger.info(f"[reCAPTCHA v2] Page dibuang (context di-restart) — {task_id}")

    # ──────────────────────────────────────────────
    #  HCAPTCHA SOLVER
    # ──────────────────────────────────────────────

    async def _solve_hcaptcha_task(self, task_id, url, sitekey, version):
        start_time = time.time()
        page, context = await self.page_pool.get()
        try:
            res = await _solve_hcaptcha_impl(page, url, sitekey, version=version)
            elapsed = res.get("elapsed", round(time.time() - start_time, 3))
            token = res.get("token")
            if token:
                self.results[task_id] = {"status": "success", "elapsed_time": elapsed, "value": token}
                logger.success(f"[hCaptcha] Sukses ({res.get('method', '')}) — {task_id} ({elapsed}s)")
            else:
                self.results[task_id] = {
                    "status": "error", "elapsed_time": elapsed,
                    "value": "captcha_fail", "message": res.get("error", "gagal")}
                logger.warning(f"[hCaptcha] Gagal — {task_id}: {res.get('error')}")
        except Exception as e:
            elapsed = round(time.time() - start_time, 3)
            self.results[task_id] = {"status": "error", "elapsed_time": elapsed, "value": str(e)}
            logger.error(f"[hCaptcha] Exception — {task_id}: {e}")
        finally:
            try:
                await page.unroute_all()
            except Exception:
            ...
            if context in self.contexts:
                ...
                logger.info(f"[hCaptcha] Page dibuang (context di-restart) — {task_id}")
    ...
    async def process_recaptcha_v2(
        self,
        url: str = Query(..., description="URL target / Domain yang menggunakan reCAPTCHA"),
        sitekey: str = Query(..., description="siteKey untuk reCAPTCHA"),
        version: str = Query("v2", description="v2 | v3 | invisible | checkbox"),
        action: str = Query("submit", description="Action name untuk reCAPTCHA (invisible/v3)"),
        enterprise: bool = Query(False, description="pakai grecaptcha.enterprise (Enterprise sitekey)"),
        classifier: str = Query("auto", description="Klasifikasi gambar: auto | onnx (model local)"),
    ):
        """Endpoint untuk memecahkan reCAPTCHA v2 checkbox / invisible + Enterprise.
        Jika challenge gambar muncul, grid diselesaikan dengan model ONNX lokal."""
        if not url or not sitekey:
            raise HTTPException(status_code=400, detail={"status": "error", "error": "Parameter 'url' dan 'sitekey' wajib diisi"})

        task_id = str(uuid.uuid4())
        self.results[task_id] = {"status": "process", "message": "solving recaptcha v2", "start_time": time.time()}
        try:
            asyncio.create_task(self._solve_recaptcha_v2_task(
                task_id, url, sitekey, version, enterprise, action, classifier))
            return JSONResponse(content={"task_id": task_id, "status": "accepted"}, status_code=202)
        except Exception as e:
            self.results.pop(task_id, None)
            return JSONResponse(content={"status": "error", "message": str(e)}, status_code=500)
    ...
    async def process_hcaptcha(
        self,
        url: str = Query(..., description="URL target / Domain yang menggunakan hCaptcha"),
        sitekey: str = Query(..., description="siteKey untuk hCaptcha"),
        version: str = Query("checkbox", description="checkbox | invisible"),
    ):
        """Endpoint untuk memecahkan hCaptcha (checkbox / invisible).
        hCaptcha dipasang via route-intercept dan token diambil dari
        [name=h-captcha-response]."""
        if not url or not sitekey:
            raise HTTPException(status_code=400, detail={"status": "error", "error": "Parameter 'url' dan 'sitekey' wajib diisi"})

        task_id = str(uuid.uuid4())
        self.results[task_id] = {"status": "process", "message": "solving hcaptcha", "start_time": time.time()}
        try:
            asyncio.create_task(self._solve_hcaptcha_task(task_id, url, sitekey, version))
            return JSONResponse(content={"task_id": task_id, "status": "accepted"}, status_code=202)
        except Exception as e:
            self.results.pop(task_id, None)
            return JSONResponse(content={"status": "error", "message": str(e)}, status_code=500)
...
"""
    ...
    print("  \033[90m PythonMagic-Solver v1.0.5\033[0m")
    print()
...
def _auto_install():
    import subprocess
    PACKAGES = ["fastapi==0.95.2", "uvicorn", "camoufox[fetch]", "loguru", "psutil", "onnxruntime", "numpy", "pillow"]
    print("\n" + "═" * 52)
...
if __name__ == "__main__":
    ...
    print("═" * 52)
    print("  🚀  Memulai PythonMagic-Solver...")
    print("═" * 52 + "\n")
